...
public class SignInData(AppDbContext ctx, IConfiguration configuration)
{
    public async Task<Token> SignIn(SignInRequest request, CancellationToken ct)
    {
        UserModel user = await ctx.Users.Include(u => u.Role).FirstOrDefaultAsync(u => u.Email == request.Email, ct) ??
